...
const outputFile = { zig: './gen.zig', h: '../emscripten/raylib_marshall_gen.h', c: '../emscripten/raylib_marshall_gen.c' };
const inputCode = inputFiles.map(inputFile => fs.readFileSync(inputFile).toString('utf-8')).join('\n');


const nameAndReturnRx = /^(?:RAYGUIAPI|RLAPI)\s+(\w+)\s+(\w+)\s*\(([A-Za-z-0-9*, ]*)\)\s*;/gm;
const nameAndReturnLineRx = /^(?:RAYGUIAPI|RLAPI)\s+(\w+)\s+(\w+)\s*\(([A-Za-z-0-9*, ]*)\)\s*;/m;
const parameterRxAll = /(?:((?:(?:const )?(?:unsigned )?)?\w+)\s+(\*?)\s*(\w+)(,|$))/g;
const parameterRx = /(?:((?:(?:const )?(?:unsigned )?)?\w+)\s+(\*?)\s*(\w+)(,|$))/;
...
const argMap = [
    ...
    { t: 'const char', p: true, z: '[]const u8', n: (a) => `@ptrCast([*c]const u8, ${a.n}.ptr)` },
    ...
    { t: 'Font', p: false, z: 't.Font', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Rectangle', p: false, z: 't.Rectangle', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Camera2D', p: false, z: 't.Camera2D', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Vector2', p: false, z: 't.Vector2', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Color', p: false, z: 't.Color', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Matrix', p: false, z: 't.Matrix', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
    { t: 'Texture2D', p: false, z: 't.Texture2D', s: true, n: (a) => `@ptrCast([*c]r.${a.t}, &_${a.n})` },
];
...
function mapFunction(m) {
    ...
    const args = hasNoParameters ? null : mParams.map(arg => {
    ...
    const zigSignature = `pub fn ${functionName} (${mappedArgs.map(a => `${a.name}: ${a.z}`).join(', ')}) ${mappedReturn.z}`;
    //--- Zig body -----------------
    let zigBody = '{';
    ...
    if (mappedReturn.s) cSignature += [mappedReturn, ...mappedArgs].map(a => `${a.t} ${a.p || a.s ? '*' : ''}${a.name}`).join(', ');
    else cSignature += mappedArgs.map(a => `${a.t} ${a.p || a.s ? '*' : ''}${a.name}`).join(', ');
...
//=== Write Zig =================================
fs.writeFileSync(outputFile.zig,
